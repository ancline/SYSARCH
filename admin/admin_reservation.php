    ...
    if ($_POST['action'] === 'approve' && $res_id > 0) {
        ...
        if ($res_row) {
            ...
            try {
                ...
                $conn->commit();
                $pc_label = $pc_num > 0 ? " PC $pc_num is now marked <strong>In Use</strong>." : '';
...
<head>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .card{background:var(--panel);border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(15,38,83,0.08);border:1px solid var(--border);margin-bottom:24px}
        ...
        .add-lab-form{display:flex;align-items:flex-end;gap:12px;flex-wrap:wrap;background:var(--bg);border:1px solid var(--border);border-radius:12px;padding:16px 18px;margin-bottom:20px}
        ...
        .add-lab-form select,.add-lab-form input[type="text"],.add-lab-form input[type="number"]{height:36px;padding:0 12px;border:1px solid var(--border);border-radius:8px;font-size:13px;font-family:'DM Sans',sans-serif;color:var(--text);outline:none;transition:border-color 0.18s,box-shadow 0.18s;background:white}
        ...
        .custom-name-wrap{display:none;flex-direction:column;gap:5px}
        ...
        .custom-name-wrap input{height:36px;padding:0 12px;width:220px;border:1px solid var(--accent);border-radius:8px;font-size:13px;font-family:'DM Sans',sans-serif;color:var(--text);outline:none;box-shadow:0 0 0 3px rgba(59,111,212,0.1);background:white;animation:slideDown 0.18s ease}
        @keyframes slideDown{from{opacity:0;transform:translateY(-4px)}to{opacity:1;transform:translateY(0)}}
        .btn-add-lab{height:36px;padding:0 20px;border:none;border-radius:8px;background:linear-gradient(135deg,var(--navy),var(--navy-light));color:white;font-size:13px;font-weight:700;font-family:'DM Sans',sans-serif;cursor:pointer;box-shadow:0 2px 8px rgba(15,38,83,0.2);transition:opacity 0.15s,transform 0.15s}
        .btn-add-lab:hover{opacity:0.88;transform:translateY(-1px)}
        .add-lab-note{width:100%;font-size:11.5px;color:var(--amber);background:var(--amber-light);border:1px solid #fde68a;border-radius:7px;padding:7px 12px;display:flex;align-items:center;gap:6px}
        .no-labs-notice{width:100%;font-size:12px;color:var(--green);background:var(--green-light);border:1px solid #b2e8cc;border-radius:7px;padding:8px 14px;display:flex;align-items:center;gap:6px;font-weight:600}
        .lab-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:14px}
        .lab-config-card{border:1px solid var(--border);border-radius:12px;background:white;overflow:hidden;box-shadow:0 2px 10px rgba(15,38,83,0.06);transition:transform 0.16s,box-shadow 0.16s;position:relative}
        ...
        .pc-setup-required-badge{position:absolute;top:10px;right:10px;background:linear-gradient(135deg,#f59e0b,#d97706);color:white;font-size:10px;font-weight:800;padding:3px 9px;border-radius:20px;text-transform:uppercase;letter-spacing:0.5px;box-shadow:0 2px 8px rgba(245,158,11,0.4);animation:pulse-badge 2s ease-in-out infinite}
        @keyframes pulse-badge{0%,100%{box-shadow:0 2px 8px rgba(245,158,11,0.4)}50%{box-shadow:0 2px 16px rgba(245,158,11,0.7)}}
        .pc-setup-done-badge{position:absolute;top:10px;right:10px;background:linear-gradient(135deg,#22c55e,#16a34a);color:white;font-size:10px;font-weight:800;padding:3px 9px;border-radius:20px;text-transform:uppercase;letter-spacing:0.5px}
        .lab-config-card.needs-setup{border-color:#fcd34d;box-shadow:0 2px 10px rgba(245,158,11,0.15)}
        .lab-setup-warning{margin:0 14px 10px;background:#fffbeb;border:1px solid #fde68a;border-radius:8px;padding:8px 12px;display:flex;align-items:center;gap:8px;font-size:11.5px;color:var(--amber);font-weight:600}
        .lab-setup-warning .btn-setup-now{margin-left:auto;height:24px;padding:0 10px;background:#f59e0b;color:white;border:none;border-radius:6px;font-size:11px;font-weight:700;font-family:'DM Sans',sans-serif;cursor:pointer;white-space:nowrap;transition:opacity 0.15s}
        .lab-setup-warning .btn-setup-now:hover{opacity:0.85}
        .lab-card-top{padding:14px 16px 10px;display:flex;align-items:flex-start;gap:12px}
        .lab-card-icon{width:40px;height:40px;border-radius:10px;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:18px;background:linear-gradient(135deg,var(--navy),var(--navy-light))}
        .lab-card-info{flex:1;min-width:0}
        .lab-card-name{font-size:14px;font-weight:700;color:var(--navy);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;padding-right:80px}
        .lab-card-meta{font-size:11.5px;color:var(--muted);margin-top:3px}
        .pc-availability-bar{margin:0 16px 10px;height:7px;background:#eef1f8;border-radius:10px;overflow:hidden}
        .pc-bar-fill{height:100%;border-radius:10px;background:linear-gradient(90deg,#22c55e,#16a34a);transition:width 0.4s}
        .lab-pc-counts{display:flex;gap:12px;padding:0 16px 10px}
        .pc-count-chip{font-size:11px;font-weight:700;padding:3px 9px;border-radius:20px}
        ...
        .lab-card-footer{border-top:1px solid var(--border);padding:9px 12px;display:flex;gap:6px;align-items:center;background:#fafbfd}
        .btn-lab-action{height:28px;padding:0 11px;border-radius:7px;font-size:11.5px;font-weight:700;font-family:'DM Sans',sans-serif;cursor:pointer;border:1px solid;transition:opacity 0.15s,transform 0.15s}
        ...
        .inactive-badge{font-size:10px;font-weight:700;padding:2px 8px;background:#f5f5f5;color:#999;border-radius:20px;border:1px solid #ddd;text-transform:uppercase;letter-spacing:0.4px;margin-left:auto}
        .active-badge{font-size:10px;font-weight:700;padding:2px 8px;background:#edfaf3;color:var(--green);border-radius:20px;border:1px solid #b2e8cc;text-transform:uppercase;letter-spacing:0.4px;margin-left:auto}
        ...
        .filter-bar{padding:14px 18px;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:10px;flex-wrap:wrap;background:#fafbfd}
        .filter-bar label{font-size:12px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:0.4px}
        .filter-bar input[type="text"],.filter-bar select{height:34px;padding:0 11px;border:1px solid var(--border);border-radius:8px;font-size:13px;font-family:'DM Sans',sans-serif;color:var(--text);background:white;outline:none;transition:border-color 0.18s,box-shadow 0.18s}
        ...
        .btn-filter{height:34px;padding:0 16px;background:var(--accent);color:white;border:none;border-radius:8px;font-size:12.5px;font-weight:700;font-family:'DM Sans',sans-serif;cursor:pointer;transition:opacity 0.15s,transform 0.15s}
        .btn-filter:hover{opacity:0.88;transform:translateY(-1px)}
        .btn-reset-filter{height:34px;padding:0 14px;border:1px solid var(--border);border-radius:8px;background:white;color:var(--muted);font-size:12px;font-weight:600;cursor:pointer;font-family:'DM Sans',sans-serif;transition:background 0.15s,color 0.15s}
        ...
        thead th{padding:11px 14px;text-align:left;font-size:11px;font-weight:700;color:var(--navy);text-transform:uppercase;letter-spacing:0.5px;white-space:nowrap}
        ...
        .lab-tag{display:inline-flex;align-items:center;gap:5px;background:var(--tag-bg);color:var(--navy-light);padding:4px 10px;border-radius:20px;font-size:12px;font-weight:700;border:1px solid #c8d4ee;white-space:nowrap;cursor:pointer;transition:background 0.15s,box-shadow 0.15s}
        .lab-tag:hover{background:#d0daf4;box-shadow:0 2px 8px rgba(36,82,160,0.15)}
        .pc-tag{display:inline-flex;align-items:center;gap:4px;background:#dce8ff;color:var(--accent);padding:3px 9px;border-radius:20px;font-size:11.5px;font-weight:700;border:1px solid #b8ccf5;white-space:nowrap;margin-top:4px}
        .pc-tag.in-use{background:#fff8e1;color:#c07000;border-color:#fcd87a}
        .slot-tag{display:inline-flex;align-items:center;background:#f0f3f9;color:var(--text);padding:4px 10px;border-radius:20px;font-size:12px;font-weight:600;border:1px solid var(--border);white-space:nowrap}
        ...
        .status-pill{display:inline-flex;align-items:center;gap:5px;padding:4px 10px;border-radius:20px;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.4px;white-space:nowrap}
        ...
        .btn-approve,.btn-reject,.btn-delete,.btn-pc{padding:5px 12px;border-radius:7px;font-size:11.5px;font-weight:700;font-family:'DM Sans',sans-serif;cursor:pointer;border:none;transition:opacity 0.15s,transform 0.15s}
        ...
        @media(max-width:1000px){.stat-strip{grid-template-columns:repeat(2,1fr)}.page-wrapper{padding:20px 16px 40px}.nav-title{display:none}}
...
<div class="page-wrapper">
    <div class="page-header">
        ...
        <p>Set PC conditions for each lab first. <strong>Approving a reservation automatically marks the chosen PC as In Use.</strong> Rejecting or deleting an approved reservation frees the PC back to Available.</p>
    </div>
...
<div class="card">
    <div class="card-header">
        ...
        <div style="font-size:11.5px;color:rgba(255,255,255,0.65);font-weight:500;">Labs open after PC conditions are published</div>
    </div>
    <div class="lab-config-body">
        ...
        <div class="lab-grid">
            ...
            <div class="lab-config-card <?= $inactive ? 'inactive' : '' ?> <?= $needs_setup ? 'needs-setup' : '' ?>">
                ...
                <?php else: ?>
                <div class="pc-availability-bar"><div class="pc-bar-fill" style="width:<?= $avail_pct ?>%"></div></div>
...
<div class="card">
    <div class="card-header">
        ...
        <div style="font-size:11.5px;color:rgba(255,255,255,0.65);font-weight:500;">Approving auto-marks the chosen PC as In Use · Rejecting/deleting an approved reservation frees the PC</div>
    </div>
    ...
    <div class="table-wrap">
        ...
        <table>
            <thead>
                <tr>
                    <th>#</th><th>Student</th><th>Laboratory / PC</th><th>Date</th>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <br><span class="pc-tag <?= $status === 'approved' ? 'in-use' : '' ?>">
                            🖥️ PC <?= htmlspecialchars($pc_num) ?><?= $status === 'approved' ? ' · In Use' : '' ?>
                        </span>
                        <?php endif; ?>
                    </td>
                    <td>
                    ...
                    <td style="max-width:180px;"><div style="font-size:13px;line-height:1.4;white-space:normal;"><?= htmlspecialchars($res['purpose']) ?></div></td>
                    <td class="center">
                    ...
                    <td class="center" style="font-size:12px;color:var(--muted);white-space:nowrap;"><?= htmlspecialchars($sub_fmt) ?></td>
                    <td class="center">
...
function openModal(action, id, name, pcNum) {
    ...
    if (action === 'approve' && pcNum) {
        noticeText.textContent = 'PC ' + pcNum + ' will automatically be marked as In Use.';
