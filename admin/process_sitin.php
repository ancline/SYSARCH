...
if (empty($student_id) || empty($purpose) || empty($lab)) {
    header('Location: /SYSARCH/admin/admin_SitIn.php?error=missing_fields');
    exit();
}
...
if (!$student) {
    header('Location: /SYSARCH/admin/admin_SitIn.php?error=student_not_found');
    exit();
}
...
if ($check->num_rows > 0) {
    $check->close();
    header('Location: /SYSARCH/admin/admin_SitIn.php?error=already_sitin');
    exit();
}
$check->close();
...
if ($current_sessions > 0) {
    ...
    if (!empty($remaining_session) && (int)$remaining_session > 0) {
        ...
        header('Location: /SYSARCH/admin/admin_SitIn.php?error=no_sessions');
        exit();
    }
}
...
if ($success) {
    ...
    header('Location: /SYSARCH/admin/admin_SitIn.php?success=1');
} else {
    header('Location: /SYSARCH/admin/admin_SitIn.php?error=insert_failed');
}
exit();