...
if (
    ...
    header('Location: ' . BASE_PATH . 'user_profile.php?error=' . urlencode('Invalid request. Please try again.'));
    exit();
}

// Only accept POST
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header('Location: ' . BASE_PATH . 'user_profile.php');
    exit();
}
...
if (empty($last_name) || empty($first_name) || empty($email) || empty($course)) {
    header('Location: ' . BASE_PATH . 'user_profile.php?error=' . urlencode('Please fill in all required fields.'));
    exit();
}

if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    header('Location: ' . BASE_PATH . 'user_profile.php?error=' . urlencode('Invalid email address.'));
    exit();
}

if (!in_array($year_level, [1, 2, 3, 4])) {
    header('Location: ' . BASE_PATH . 'user_profile.php?error=' . urlencode('Invalid year level selected.'));
    exit();
}
...
if (!empty($password)) {
    if (strlen($password) < 6) {
        header('Location: ' . BASE_PATH . 'user_profile.php?error=' . urlencode('Password must be at least 6 characters.'));
        exit();
    }
    if ($password !== $password_confirm) {
        header('Location: ' . BASE_PATH . 'user_profile.php?error=' . urlencode('Passwords do not match.'));
        exit();
    }
}
...
if ($stmt->execute()) {
    ...
    header('Location: ' . BASE_PATH . 'user_profile.php?error=' . urlencode('Update failed. Please try again.'));
}
